...
class VTSum_BLIP_TT(nn.Module):
    ...
    def generate(self, video_embeddings, video_mask, sample=False,
             num_beams=1, max_length=30, min_length=10, top_p=0.9,
             repetition_penalty=1.0):
        device = video_embeddings.device
        batch_size = video_embeddings.size(0)

        # ensure video_mask dtype + device
        video_mask = video_mask.to(device=device, dtype=torch.long)
        ...
        outputs = self.text_decoder.generate(**generate_kwargs)
