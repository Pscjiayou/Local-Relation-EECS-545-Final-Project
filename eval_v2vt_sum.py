...
import utils
from utils import update_config, compute_f1, compute_kendall, compute_spearman, concat_all_gather
...
def evaluate(model, clip_model, data_loader, device, config, eval_type):
    ...
    try:
        ...
        f1_max_score = f1_max_score.mean().item()
        f1_mean_score = f1_mean_score.mean().item()
        kendall_mean_score = kendall_mean_score.mean().item()
        spearman_mean_score = spearman_mean_score.mean().item()
        clipscores = clipscores.mean().item()
    ...
    if utils.is_main_process():
        ...
        coco_eval.evaluate(['Bleu', 'ROUGE_L', 'CIDEr'])
